...
struct Node {
    int id;
    string name;
    int type;  // 0=depot, 1=customer, 2=fuel_station, 3=waypoint
...
public:
    ...
    void set_solution(vector<int> final_route, int final_cost) {
        ...
        if(route.empty()) current_location = 0;
        else current_location = route.back();
    }
...
private:
    ...
    vector<int> waypoints;       // list of waypoint node ids
...
public:
    ...
    void set_waypoints(vector<int> waypoint_list){
        ...
        while (!pq.empty()) {
            ...
            for (auto& edge : graph->adj_list[current.location]) {
                ...
                if (current.fuel >= travel_cost) {
                    ...
                    bool is_waypoint = find(waypoints.begin(), waypoints.end(), next_location) != waypoints.end();
                    ...
                    if (is_waypoint) {
                        cout << "Passing through waypoint: " << graph->nodes[next_location].name
                            << " (cost: " << travel_cost << ", fuel left: " << next_state.fuel << ")" << endl;
                    }
...
int main() {
    ...
    graph.add_edge(0, 1, 8);  // Depot to Customer1 (direct, longer)
    graph.add_edge(0, 4, 5);   // Depot to Waypoint1 (shorter)
    graph.add_edge(4, 1, 6);   // Waypoint1 to Customer1 (total: 5+6=11, shorter than direct!)
    graph.add_edge(0, 2, 12);  // Depot to Customer2 (direct)
    graph.add_edge(1, 5, 3);   // Customer1 to Waypoint2
    graph.add_edge(5, 2, 4);   // Waypoint2 to Customer2 (via waypoint: 3+4=7)
    graph.add_edge(2, 3, 2);   // Customer2 to Fuel station
    graph.add_edge(1, 3, 8);   // Customer1 to Fuel station (direct)
    graph.add_edge(4, 3, 7);   // Waypoint1 to Fuel station
    
    // Print graph
    graph.print_graph();
    
    // Create vehicle
    Vehicle vehicle(27, 0);  // fuel_capacity=11, start_at_depot=0
